...
sys.path.append(os.path.abspath('../py-tab/'))
...
try:
  serial_port = serial.Serial(port=dev,baudrate=9600)
except:
...
# 1. Test Common Ack
cmd = TxCmd(COMMON_ACK_OPCODE, HWID, msgid, GND, COM)
byte_i = 0
...
# 2. Test Common Nack
cmd = TxCmd(COMMON_NACK_OPCODE, HWID, msgid, GND, COM)
byte_i = 0
while rx_cmd_buff.state != RxCmdBuffState.COMPLETE:
  if byte_i < cmd.get_byte_count():
    serial_port.write(cmd.data[byte_i].to_bytes(1, byteorder='big'))
...
# 3. Test Common Debug
cmd = TxCmd(COMMON_DEBUG_OPCODE, HWID, msgid, GND, COM)
...
cmd = TxCmd(COMMON_DATA_OPCODE, HWID, msgid, GND, COM)
...
cmd = TxCmd(BOOTLOADER_ACK_OPCODE, HWID, msgid, GND, COM)
byte_i = 0
while rx_cmd_buff.state != RxCmdBuffState.COMPLETE:
  if byte_i < cmd.get_byte_count():
    serial_port.write(cmd.data[byte_i].to_bytes(1, byteorder='big'))
...
cmd = TxCmd(BOOTLOADER_NACK_OPCODE, HWID, msgid, GND, COM)
byte_i = 0
while rx_cmd_buff.state != RxCmdBuffState.COMPLETE:
  if byte_i < cmd.get_byte_count():
    serial_port.write(cmd.data[byte_i].to_bytes(1, byteorder='big'))
...
cmd = TxCmd(BOOTLOADER_PING_OPCODE, HWID, msgid, GND, COM)
byte_i = 0
while rx_cmd_buff.state != RxCmdBuffState.COMPLETE:
  if byte_i < cmd.get_byte_count():
    serial_port.write(cmd.data[byte_i].to_bytes(1, byteorder='big'))
...
#   reason
cmd = TxCmd(BOOTLOADER_ERASE_OPCODE, HWID, msgid, GND, COM)
byte_i = 0
while rx_cmd_buff.state != RxCmdBuffState.COMPLETE:
  if byte_i < cmd.get_byte_count():
    serial_port.write(cmd.data[byte_i].to_bytes(1, byteorder='big'))
...
#    Number reason
cmd = TxCmd(BOOTLOADER_WRITE_PAGE_OPCODE, HWID, msgid, GND, COM)
...
#    with Address reason
#cmd = TxCmd(BOOTLOADER_WRITE_PAGE_ADDR32_OPCODE, HWID, msgid, GND, COM)
...
cmd = TxCmd(BOOTLOADER_JUMP_OPCODE, HWID, msgid, GND, COM)
byte_i = 0
while rx_cmd_buff.state != RxCmdBuffState.COMPLETE:
  if byte_i < cmd.get_byte_count():
    serial_port.write(cmd.data[byte_i].to_bytes(1, byteorder='big'))
